...
@interface ZSLabelButton : UIButton
@property (nonatomic, assign) NSInteger componentIndex;
@property (nonatomic) NSURL *url;
@end

@implementation ZSLabelButton
@end

@implementation ZSLabelComponent
...
@implementation ZSLabelExtractedComponent
...
{
    ZSLabelExtractedComponent *component = [[ZSLabelExtractedComponent alloc] init];
...
- (void)render
{
	...
	for (ZSLabelComponent *component in textComponents)
	{
		...
		else if ([component.tagLabel caseInsensitiveCompare:@"a"] == NSOrderedSame)
		{
			if (self.currentSelectedButtonComponentIndex==index)
			{
				...
				{
					//[self applyBoldStyleToText:attrString atPosition:component.position withLength:[component.text length]];
                    //					[self applyColor:@"blue" toText:attrString atPosition:component.position withLength:[component.text length]];
				}
			}
			else
			{
	...
	if (self.currentSelectedButtonComponentIndex==-1)
	{
		...
		for (ZSLabelComponent *linkableComponents in links)
		{
			float height = 0.0;
			...
			for (CFIndex i=0; i<CFArrayGetCount(frameLines); i++)
			{
				...
				if ( (linkableComponents.position<lineRange.location && linkableComponents.position+linkableComponents.text.length>(u_int16_t)(lineRange.location)) || (linkableComponents.position>=lineRange.location && linkableComponents.position<lineRange.location+lineRange.length))
				{
					...
					ZSLabelButton *button = [[ZSLabelButton alloc] initWithFrame:CGRectMake(primaryOffset+origin.x, height, button_width, ascent+descent)];
...
{
	ZSLabelButton *button = (ZSLabelButton*)sender;
...
- (void)onButtonPressed:(id)sender
{
	...
	if ([self.delegate respondsToSelector:@selector(ZSLabel:didSelectLinkWithURL:)])
	{
		[self.delegate ZSLabel:self didSelectLinkWithURL:button.url];
	}
}
...
- (void)setHighlightedText:(NSString *)text
{
	...
	ZSLabelExtractedComponent *component = [ZSLabel extractTextStyleFromText:_highlightedText paragraphReplacement:self.paragraphReplacement];
...
- (void)setText:(NSString *)text
{
	...
	ZSLabelExtractedComponent *component = [ZSLabel extractTextStyleFromText:_text paragraphReplacement:self.paragraphReplacement];
...
+ (ZSLabelExtractedComponent*)extractTextStyleFromText:(NSString*)data paragraphReplacement:(NSString*)paragraphReplacement
{
	...
	while (![scanner isAtEnd])
    {
		...
		if ([text rangeOfString:@"</"].location==0)
		{
			...
			if (position!=NSNotFound)
			{
				...
				{
					ZSLabelComponent *component = [components objectAtIndex:i];
		...
		else
		{
			...
			ZSLabelComponent *component = [ZSLabelComponent componentWithString:nil tag:tag attributes:attributes];
    ...
    return [ZSLabelExtractedComponent ZSLabelExtractComponentsWithTextComponent:components plainText:data];
}
	...
	while([scanner isAtEnd] == NO)
	{
		...
		if([valid_tags containsObject:tag] == NO)
		{
			...
			if (position!=NSNotFound)
			{
				...
				{
					// is inside a tag
					[components addObject:[ZSLabelComponent componentWithString:text2 tag:tag attributes:lastAttributes]];
				}
				else
				{
					// is outside a tag
					[components addObject:[ZSLabelComponent componentWithString:text2 tag:nil attributes:lastAttributes]];
				}
				data = [data stringByReplacingOccurrencesOfString:delimiter withString:@"" options:NSCaseInsensitiveSearch range:NSMakeRange(last_position, position+delimiter.length-last_position)];
...
+ (NSDictionary*)preExtractTextStyle:(NSString*)data
{
    ...
    ZSLabelExtractedComponent *component = [ZSLabel extractTextStyleFromText:data paragraphReplacement:paragraphReplacement];
